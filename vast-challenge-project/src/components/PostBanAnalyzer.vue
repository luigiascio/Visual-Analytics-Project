...
function processData(data) {
    ...
    Object.values(data).flat().forEach(ping => {
        ...
        const period = date <= BAN_DATE ? 'pre' : 'post';
...
function drawChart() {
    ...
    // Aumentato margin.top per fare spazio alle etichette di direzione
    ...
    // --- NUOVE ETICHETTE DIREZIONALI ---
    // Etichetta Sinistra (Diminuzione)
    svg.append("text")
        .attr("x", 0)
        .attr("y", -10)
